...
try {
  ...
  if (fs.existsSync(p)) sa = JSON.parse(fs.readFileSync(p, 'utf8'))
} catch {}
const projectId = (sa?.project_id || sa?.projectId || sa?.projectid || FIREBASE_PROJECT_ID || '')
const clientEmail = (sa?.client_email || sa?.clientEmail || CLIENT_EMAIL || '')
const privateKey = ((sa?.private_key || sa?.privateKey || PRIVATE_KEY || '')).replace(/\\n/g, '\n')
...
async function main() {
  // 用 collectionGroup 尋找所有 profile，不需要 users 根文件存在
